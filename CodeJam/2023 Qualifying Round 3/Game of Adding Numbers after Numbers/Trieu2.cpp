...
Node *rightRotate(Node *y)
{
    ...
    //cout << "rightRotate " << x ->key << ' ' << s1 << ' ' << y ->key << ' ' << s2 << endl;
	// Perform rotation
...
void preOrder(Node *root)
{
	...
	{
		preOrder(root->left);
		cout << root-> key << " " << root -> repeat << " " << root -> size << " " << root-> sum << endl;
        preOrder(root->right);
	}
}
...
{
    //cout << "sortArraytoBST " << start << ' ' << end << endl;
...
int main()
{
    ...
    while(t--) {
        ...
        cout << '\n';
        //clear(root);
        //cout << "create " << ne << "node, delete " << de << endl;
    }
